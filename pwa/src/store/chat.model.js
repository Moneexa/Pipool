...
export const ChatModel = {
    ...
    changeActiveRoom: thunk(async (actions, { id }) => {
        try {
            const {data} = await axios.get(`${config.apiUrl}/chats/${id}`);
...
socket.on('refreshRooms', (data) => {
    ...
    if (data && data.length > 0 && data[0].id) {
        store.dispatch({ type: '@action.chats.setActiveRoom', payload: { id: data[0].id } })
    }
});
